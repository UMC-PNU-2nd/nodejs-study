...
public class UserController {
    ...
    public BaseResponse<UserDto.ReadUserRes> getUsers(@RequestParam(required = true) String Email) {
        try{
            ...
            User user = userProvider.getUsersByEmail(Email);
            UserDto.ReadUserRes readUserRes = new UserDto.ReadUserRes(user);
    ...
    public BaseResponse<UserDto.ReadUserRes> getUserByIdx(@PathVariable("userIdx")Long userIdx) {
        try {
            User user = userProvider.getUsersByIdx(userIdx);
            UserDto.ReadUserRes readUserRes = new UserDto.ReadUserRes(user);
            return new BaseResponse<>(readUserRes);
        } catch(BaseException exception) {
            return new BaseResponse<>((exception.getStatus()));
    ...
    public BaseResponse<UserDto.AuthUserRes> createUser(@RequestBody UserDto.userReq req) {
        // TODO: email 관련한 짧은 validation 예시입니다. 그 외 더 부가적으로 추가해주세요!
        ...
        try{
            UserDto.AuthUserRes authUserRes = userService.createUser(req);
            return new BaseResponse<>(authUserRes);
        } catch(BaseException exception){
            return new BaseResponse<>((exception.getStatus()));
        }
    }
     ...
     */
    @PostMapping("/login")
    public BaseResponse<UserDto.AuthUserRes> loginUser(@RequestBody UserDto.LoginReq req) {
        if(req.getEmail() == null) {
            return new BaseResponse<>(POST_USERS_EMPTY_EMAIL);
        }
        // 이메일 정규표현
        ...
        if(req.getPassword() == null) {
            return new BaseResponse<>(LOGIN_PW_EMPTY);
        }
        try {
            UserDto.AuthUserRes authUserRes = userService.loginUser(req);
            return new BaseResponse<>(authUserRes);
        } catch (BaseException exception) {
            return new BaseResponse<>((exception.getStatus()));
        }
